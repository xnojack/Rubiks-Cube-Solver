...
def howClose(movesSolve):
	count = 0
	space = " "
	string = space.join(movesSolve)
	...
	for face in faces:
		for i in range(0,3):
			for y in range(0,3):
				if(cube.get_face(face)[i][y] == solved.get_face(face)[i][y]):
					count = count + 1
	return count
...
def nextGen(population,generation,gen):
	start = time.time()
	...
	if generation < 50+gen:
		return nextGen(nextGeneration,generation+1,gen)
	else:
		return nextGeneration

def initialize(population,gen):
	if len(population) < 1:
		print("Making new population ")
		population = createPopulation(int(sys.argv[3]))
		print("Created population ")
	nextGeneration = nextGen(population,gen,gen)
	return nextGeneration
...
def multiPop(number,everyone,lastGen,gen,run):
	...
	if(len(everyone) < 1):
		newPop = initialize([],gen)
	else:
		newPop = initialize(everyone,gen)

	
	ranked = fitness(newPop)
	print(ranked[0][1])
	if (len(lastGen) < 3 or lastGen.count(ranked[0][0]) < 3):
		lastGen.insert(0,ranked[0][0])
		if(len(lastGen) > 3):
			del lastGen[-1]
		return multiPop(number,newPop,lastGen,gen+50,run)
	else:
